...
-- Fix: PostgreSQL doesn't support SELECT in triggers, only INSERT/UPDATE/DELETE

-- Enhanced security audit for profiles table access (INSERT/UPDATE/DELETE only)
...
AS $$
BEGIN
  -- Log profile modification attempts with enhanced metadata
  INSERT INTO public.security_alerts (
    ...
    auth.uid(),
    'profile_table_direct_modification',
    CASE 
      WHEN TG_OP IN ('INSERT', 'UPDATE') THEN 'medium'
      WHEN TG_OP = 'DELETE' THEN 'high'
      ELSE 'medium'
    END,
    'Direct profile table modification detected - recommend using secure functions',
    jsonb_build_object(
      'operation', TG_OP,
      'table', TG_TABLE_NAME,
      'timestamp', now(),
      'user_role', COALESCE((SELECT role FROM public.profiles WHERE id = auth.uid()), 'unknown'),
      'recommendation', 'Use secure profile update functions instead',
